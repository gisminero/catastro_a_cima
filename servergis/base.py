...
# Inserta los registros traidos del WebService de la provincia de Jujuy
    def insertListaToTabla(self, cod_prov, tabla, listaTabla):
        ...
        for index in range(len(listaTabla)):
            ...
            create_date = listaTabla[index]['create_date']
            write_date = listaTabla[index]['write_date']
            #print (("NUEVOS DATOS: " + str(create_date)+ " DIA DE MODIFICACION " + str(write_date)))
            ##A continuacion obtener la parte del Json que contiene el poligono
            ##tipo de poligono y puntos en formato WKT
            ...
            #poligGeo = self._convMultiGeog(cordenadas_multipo)
            codProv = cod_prov
            query = """ INSERT INTO public."""+tabla+""" ("geom", "expediente", "nombre", "titular", "mineral", "codprov", "create_date", "write_date")
                       VALUES(ST_GeogFromText(%s),%s,%s,%s,%s,%s,to_timestamp(%s, 'YYYY-MM-DD HH24:MI:SS'),to_timestamp(%s, 'YYYY-MM-DD HH24:MI:SS')); """
            #data = (str(poligGeo), expediente,nombre,titular,mineral,codProv)
            data = (cordenadas_multipo, expediente,nombre,titular,mineral,codProv, create_date, write_date)
            try:
                self.cursor.execute(query,data)
                self.conn.commit()
            except psycopg2.IntegrityError as err:
                if err.pgcode != '23505':
                    print(('FunctionERROR: %s', err))
            except Exception as err:
                #print((('Error al insertar registro! - ')).format(err))
                print(('FuncionERROR: %s', err))
                #sys.exit(1)
            #else:
